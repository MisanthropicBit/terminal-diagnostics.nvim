...
function MultilineMatcher:match(options)
    ...
    if self.last_match_pos then
        match = patterns.find_at_cursor(buffer, self.error_spec.pattern)
        ...
        if match then
            ...
            for idx, spec in ipairs(self:specs()) do
                if idx ~= spec_idx then
                    ...
                    local _match = patterns.find(buffer, spec, dir)
        ...
        local on_header =
            patterns.find_at_cursor(buffer, self.header_spec.pattern)
        ...
        if on_header then
            ...
            if include_header then
                ...
                match, spec =
                    patterns.find(buffer, self.error_spec, count),
                    self.error_spec

                if match then
                    table.insert(results, {
                        spec = self.error_spec,
                        match = patterns.find(buffer, self.error_spec, count),
                    })
                end
            end
...
function MultilineMatcher:find_match_start(options)
    ...
    local error_line_pos = patterns.find(buffer, self.error_spec, count)

    if include_header then
        local header_pos = patterns.find(buffer, self.header_spec, count)
...
function MultilineMatcher:match_at_cursor(options)
    for _, spec in ipairs(self:specs()) do
        if matcher_utils.spec_has_info(spec) then
            local match = patterns.find_at_cursor(options.buffer, spec)
