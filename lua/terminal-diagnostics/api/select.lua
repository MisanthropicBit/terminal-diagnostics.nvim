...
local function handle_outer_select(cursor_result, options)
    ...
    if cursor_result then
        ...
        if parse_result then
            ...
            return cursor_result, {
                start = cursor_result.results[1].from,
                end_ = context.range.end_,
            }
        else
            return cursor_result, {
                start = cursor_result.results[1].from,
                end_ = cursor_result.results[1].to,
            }
        end
    end
...
function select.select(options)
    ...
    local from, to = result.results[1].from, result.results[1].to

    if _options.outer then
        ---@cast _range -nil
        from, to = _range.start, _range.end_
    end
